...
class RUN:
    ...
    def sign(self):
        ...
        if response.get("success") == True:
            ...
            if response.get("obj") and response["obj"].get(
                "integralTaskSignPackageVOList"
            ):
                packet_name = response["obj"]["integralTaskSignPackageVOList"][0][
                    "packetName"
                ]
                Log(
                    f">>>签到成功，获得【{packet_name}】，本周累计签到【{count_day + 1}】天"
                )
            else:
    ...
    def superWelfare_receiveRedPacket(self):
        ...
        if response.get("success") == True:
            ...
            error_message = (
                response.get("errorMessage") or json.dumps(response) or "无返回"
            )
    ...
    def receive_honeyTask(self):
        ...
        if response.get("success") == True:
            ...
            print(
                f'收取任务【{self.taskType}】失败！原因：{response.get("errorMessage")}'
            )
    ...
    def EAR_END_2023_TaskList(self):
        print("\n>>>>>>开始年终集卡任务")
        # 任务列表
        json_data = {"activityCode": "YEAR_END_2023", "channelType": "MINI_PROGRAM"}
        ...
        for l in range(10):
            for i in range(0, 3):
                ...
                if response.get("success") == True:
                    ...
                    for card in receivedAccountList:
                        print(
                            f'>获得：【{card["currency"]}】卡【{card["amount"]}】张！'
                        )
        ...
        if response.get("success") == True:
            ...
            for weekly_gift in weekly_gift_list:
                if not weekly_gift.get("received"):
                    receive_start_time = datetime.strptime(
                        weekly_gift["receiveStartTime"], "%Y-%m-%d %H:%M:%S"
                    )
                    receive_end_time = datetime.strptime(
                        weekly_gift["receiveEndTime"], "%Y-%m-%d %H:%M:%S"
                    )
                    current_time = datetime.now()
                    # print(current_time)
    ...
    def anniversary2024_receive_weekly_gift(self):
        ...
        if response.get("success"):
            product_names = [
                product["productName"] for product in response.get("obj", [])
            ]
    ...
    def anniversary2024_taskList(self):
        url = "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~activityTaskService~taskList"
        data = {"activityCode": "ANNIVERSARY_2024", "channelType": "MINI_PROGRAM"}
        response = self.do_request(url, data)
        ...
        if response and response.get("success"):
            ...
            print(
                ...
                % (
                    ...
                    or (json.dumps(response) if response else "无返回"),
                )
            )
    ...
    def anniversary2024_fetchMixTaskReward(self, task):
        ...
        if response and response.get("success"):
            ...
            received_list = [
                f"[{item['currency']}]X{item['amount']}"
                for item in reward_info.get("receivedAccountList", [])
            ]
    ...
    def anniversary2024_unbox(self):
        ...
        if response and response.get("success"):
            ...
            unbox_list = [
                f"[{item['currency']}]X{item['amount']}"
                for item in account_info.get("receivedAccountList", [])
            ]
    ...
    def anniversary2024_game_list(self):
        ...
        try:
            if response["success"]:
                ...
                if not search_word_info.get("isPassFlag") or not search_word_info.get(
                    "isFinishDailyFlag"
                ):
                    print("开始找字游戏")
                    for i in range(1, 11):
                ...
                if not happy_elimination_info.get(
                    "isPassFlag"
                ) or not happy_elimination_info.get("isFinishDailyFlag"):
                    print("开始消消乐")
                    for i in range(1, 31):
                ...
                error_message = (
                    response["errorMessage"] or json.dumps(response) or "无返回"
                )
    ...
    def anniversary2024_SearchWord_win(self, index):
        ...
        try:
            ...
            if response and response.get("success"):
                ...
                print(
                    f'找字游戏第{index}关通关成功: {rewards if rewards else "未获得奖励"}'
                )
            else:
                error_message = (
                    response.get("errorMessage") or json.dumps(response) or "无返回"
    ...
    def anniversary2024_HappyElimination_win(self, index):
        ...
        try:
            if response and response.get("success"):
                ...
                rewards = ", ".join(
                    [
                        f"[{c.get('currency')}]X{c.get('amount')}"
                        for c in currency_dto_list
                    ]
                )
    ...
    def anniversary2024_TopicPk_chooseSide(self, index):
        ...
        try:
            if response and response.get("success"):
                ...
                print(
                    f'话题PK赛选择话题{index}成功： {rewards if rewards else "未获得奖励"}'
                )
            else:
                error_message = (
                    response["errorMessage"] or json.dumps(response) or "无返回"
    ...
    def anniversary2024_TopicPk_chooseSide(self, index):
        ...
        if result and result.get("success"):
            ...
            if currency_dto_list:
                rewards = [
                    f"[{currency['currency']}]{currency['amount']}次"
                    for currency in currency_dto_list
                ]
    ...
    def anniversary2024_titleList(self):
        ...
        if response and response.get("success"):
            ...
            guess_title_info_list = response.get("obj", {}).get(
                "guessTitleInfoList", []
            )
            today_titles = [
                title
                for title in guess_title_info_list
                if title["gameDate"] == self.today
            ]
        ...
        if response and response.get("success"):
            ...
            for award_info in today_awards:
                if award_info["answerStatus"]:
                    awards = award_info.get("awardList", []) + award_info.get(
                        "puzzleList", []
                    )
    ...
    def anniversary2024_queryAccountStatus(self):
        ...
        if result.get("success"):
            ...
            unbox_chance_currency = [
                currency
                for currency in account_currency_list
                if currency.get("currency") == "UNBOX_CHANCE"
            ]
            unbox_chance_balance = (
                unbox_chance_currency[0].get("balance") if unbox_chance_currency else 0
            )
        ...
        if result.get("success"):
            ...
            if account_currency_list:
                ...
                for card in cards_li:
                    ...
                    card_info.append(
                        "[" + card.get("currency") + "]X" + str(card.get("balance"))
                    )
    ...
    def can_draw(self, cards, n):
        # 判断是否有足够的不同卡进行抽奖
        distinct_cards = sum(
            1 for card, amount in cards.items() if card != "COMMON_CARD" and amount > 0
        )
    ...
    def member_day_index(self):
        ...
        try:
            ...
            if response.get("success"):
                ...
                can_receive_invite_award = response.get("obj", {}).get(
    ...
    def member_day_fetch_mix_task_reward(self, task):
        try:
            ...
            if response.get("success"):
                ...
                error_message = response.get("errorMessage", "无返回")
                Log(
                    "领取会员日任务[" + task["taskName"] + "]奖励失败: " + error_message
                )
                if "没有资格参与活动" in error_message:
                    self.member_day_black = True
                    Log("会员日任务风控")
        ...
        try:
            ...
            if response.get("success"):
                ...
                if self.member_day_red_packet_map.get(self.max_level):
                    Log(
                        f"会员日已拥有[{self.max_level}级]红包X{self.member_day_red_packet_map[self.max_level]}"
                    )
                    self.member_day_red_packet_draw(self.max_level)
                else:
                    remaining = self.packet_threshold - remaining_needed
                    Log(
                        f"会员日距离[{self.max_level}级]红包还差: [1级]红包X{remaining}"
                    )
    ...
    def member_day_red_packet_draw(self, level):
        try:
            ...
            if response and response.get("success"):
                coupon_names = [
                    item["couponName"] for item in response.get("obj", [])
                ] or []
    ...
    def DRAGONBOAT_2024_index(self):
        ...
        try:
            ...
            self.headers["referer"] = (
                f"https://mcs-mimp-web.sf-express.com/origin/a/mimp-activity/dragonBoat2024?mobile={self.mobile}&userId={self.user_id}&path=/origin/a/mimp-activity/dragonBoat2024&supportShare=&inviteUserId={invite_user_id}&from=newExpressWX"
            )
            payload = {}
            ...
            if response.get("success"):
                ...
                for gift in obj:
                    ...
                    if received:
                        ...
                        continue
                    receive_start_time = datetime.strptime(
                        receiveStartTime, "%Y-%m-%d %H:%M:%S"
                    )
                    receive_end_time = datetime.strptime(
                        receiveEndTime, "%Y-%m-%d %H:%M:%S"
                    )
                    is_within_range = (
                        receive_start_time <= datetime.now() <= receive_end_time
                    )
                    if is_within_range:
                        print(f">> 开始领取礼包：")
    ...
    def DRAGONBOAT_2024_taskList(self):
        print("====== 查询推币任务列表 ======")
        try:
            payload = {"activityCode": "DRAGONBOAT_2024", "channelType": "MINI_PROGRAM"}
            url = "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~activityTaskService~taskList"
    ...
    def DRAGONBOAT_2024_finishTask(self):
        try:
            payload = {"taskCode": self.taskCode}
            url = "https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberEs~taskRecord~finishTask"
...
def get_quarter_end_date():
    ...
    # 计算下个季度的第一天
    next_quarter_first_day = datetime(
        current_year, ((current_month - 1) // 3 + 1) * 3 + 1, 1
    )
