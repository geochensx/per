...
async function main() {
    try {
        ...
        $.log(`?? a total of ${userCookie?.length ?? 0} accounts were identified during this operation.`);
        let index = 0;
        //doTask of userList
        for (let user of userCookie) {
            ...
            if ($.ckStatus) {
                ...
                DoubleLog(`?? 「${user.userName ?? `账号${index}`}」check ck error!`)
            }
            //notify
